...
new g_vault
new g_rounds
new g_weaponsdisabled
new g_HUDMsgSync
new g_maxplayers

public plugin_init()
{
	register_plugin("KVIP 2", "1.0", "FLWL")
...
public event_DeathMsg()
{
	...
	{
		static iMoney, iHealth, iHPMax, iHS
...
{
	if (!is_user_alive(id) || !get_pcvar_num(cvar_weaponsenabled) || g_weaponsdisabled)
		return;
	
	if (get_user_flags(id) & VIP_FLAG)
	{
		if (!user_has_weapon(id, CSW_HEGRENADE)) give_item(id, "weapon_hegrenade")
		if (!user_has_weapon(id, CSW_FLASHBANG)) give_item(id, "weapon_flashbang")
		if (!user_has_weapon(id, CSW_SMOKEGRENADE)) give_item(id, "weapon_smokegrenade")
...
public clcmd_sayvipmenu(id)
{
	if (get_user_flags(id) & VIP_FLAG)
		...
		client_print(id, print_chat, "^x04[KVIP 2]^x01 You are not a^x04 VIP^x01.")
}
...
show_menu_primary(id)
{
	...
	len = 0
	menukeys = MENU_KEY_1|MENU_KEY_2|MENU_KEY_9|MENU_KEY_0
	
	// Title
	len += formatex(menu, charsmax(menu) - len, "\yVIP Weapon Menu^n^n")
	
	// 1. M4A1 Carbine
	len += formatex(menu[len], charsmax(menu) - len, "\r1.\w M4A1 Carbine^n")
	...
	if ((g_rounds % 3) == 1)
	{
		...
		len += formatex(menu[len], charsmax(menu) - len, "\d3. AWP Magnum Sniper^n")
	
	// Spacers
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^n^n^n")
	
	// 9. Next
	len += formatex(menu[len], charsmax(menu) - len, "\r9.\w Next")
	
	// 0. Exit
	len += formatex(menu[len], charsmax(menu) - len, "^n^n\r0.\w Exit")
	
	show_menu(id, menukeys, menu, 15, "Primary Menu")
}
...
check_weapons()
{
	static configsdir[64], mapsfile[64], curmap[32]
	get_configsdir(configsdir, charsmax(configsdir))
	get_mapname(curmap, charsmax(curmap))
	formatex(mapsfile, charsmax(mapsfile), "%s/noweapons.ini", configsdir)
	...
	if (file_exists(mapsfile))
	{
		...
		while (!feof(file))
		{
			...
			if (equali(linedata, curmap))
			{
				g_weaponsdisabled = true
